...
class Preprocesser():
    ...
    def __init__(self, file_train, file_dev, save_dir='./',
        ...
        self.train0_size = train0_size
        self.traindev_size = 1 - train0_size

        self.train1_size = train1_size
    ...
    def filter_dev(self):
        f0 = self.data_dev.N >=self.min_N
        f1 = self.data_dev.N <=self.max_N
        filter_ = f0 & f1
        self.data_dev = self.data_dev[filter_]
    ...
    def read_datasets(self):
        ...
        self.data_dev = pd.read_csv(self.file_dev, usecols=['ID', 'Address','Class','N'])

        # Extract classes and the number of them
        self.classes = list(set(self.data_dev.Class))
        self.num_classes = len(self.data_dev)
        
        # Filter train according to number of observations and elements per class
        self.filter_train()
        self.filter_dev()

        # Join all data in one df
        self.data = pd.concat([self.data_train, self.data_dev])
    ...
    def indices(self, train_ids, traindev_ids, dev_ids, test_ids):
        ind = range(len(self.IDs))
        ind_dict = dict(zip(self.IDs, ind))
        ind_train = list(map(ind_dict.get, train_ids))
        ind_traindev = list(map(ind_dict.get, traindev_ids))
        ind_dev = list(map(ind_dict.get, dev_ids))
        ind_test = list(map(ind_dict.get, test_ids))

        return ind_train, ind_traindev, ind_dev, ind_test
    ...
    def split_train_dev(self):
        ...
        self.data_dev = self.data_dev.set_index('ID')

        train_ids0, traindev_ids = train_test_split(
            self.data_train.index.values, 
            train_size=self.train0_size, 
            stratify=self.data_train.Class)
        train_ids1, test_ids = train_test_split(
            self.data_dev.index.values, 
            train_size=self.train1_size, 
            stratify=self.data_dev.Class)
        dev_ids, test_ids = train_test_split(
            test_ids, 
            train_size=self.dev_size/(1-self.train1_size),
            stratify=self.data_dev.loc[test_ids].Class)
        train_ids = np.concatenate([train_ids0, train_ids1])

        ind_train, ind_traindev, ind_dev, ind_test = self.indices(train_ids, traindev_ids, dev_ids, test_ids)

        self.Matrices_train = self.Matrices[ind_train]
        self.Labels_train = self.Labels[ind_train]
        self.IDs_train = self.IDs[ind_train]

        self.Matrices_traindev = self.Matrices[ind_traindev]
        self.Labels_traindev = self.Labels[ind_traindev]
        self.IDs_traindev = self.IDs[ind_traindev]

        self.Matrices_dev = self.Matrices[ind_dev]
        self.Labels_dev = self.Labels[ind_dev]
        self.IDs_dev = self.IDs[ind_dev]

        self.Matrices_test = self.Matrices[ind_test]
        self.Labels_test = self.Labels[ind_test]
        self.IDs_test = self.IDs[ind_test]

        splits_labels = [self.Labels_train, self.Labels_traindev, self.Labels_dev, self.Labels_test]
    ...
    def serialize_all(self):
        ...
        with open(self.save_dir+'Traindev.tfrecord', 'w') as f:
            writer = tf.io.TFRecordWriter(f.name)
            for i in range(len(self.IDs_traindev)):
                ex = self.__func_serialize(self.Matrices_traindev[i], self.Labels_traindev[i],self.IDs_traindev[i])
                writer.write(ex.SerializeToString())

        with open(self.save_dir+'Dev.tfrecord', 'w') as f:
            writer = tf.io.TFRecordWriter(f.name)
            for i in range(len(self.IDs_dev)):
                ex = self.__func_serialize(self.Matrices_dev[i], self.Labels_dev[i], self.IDs_dev[i])
                writer.write(ex.SerializeToString())
    ...
    def serialize_inference(self, save_path=None):
        ...
        with open(save_path, 'w') as f:
            ...
            for i in range(len(self.IDs)):
                ex = self.__func_serialize(self.Matrices[i], self.Labels[i],self.IDs[i])
                writer.write(ex.SerializeToString())
